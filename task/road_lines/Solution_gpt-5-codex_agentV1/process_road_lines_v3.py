...
def extract_lane_features(frame: np.ndarray, roi_mask: np.ndarray) -> np.ndarray:
    ...
    low_sat_mask = cv2.inRange(sat, 0, 150)
...
def extend_line(line_params: Optional[Tuple[float, float]], height: int, y_ratio: float = 0.65):
...
if __name__ == "__main__":
    ...
    output_video = base_dir / "gemini_solution_3.mp4"
    output_csv = base_dir / "gemini_solution_3.csv"
